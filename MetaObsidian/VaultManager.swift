...
// Writes directly into the Obsidian vault as files — no app-switching, no
// foreground requirement. Replaces an earlier Obsidian-URI-based approach that
// turned out to fail whenever this app wasn't already in the foreground (the
// normal case for wake-word-triggered, hands-free conversations), since iOS
// won't let a backgrounded app force a switch to another app.
@MainActor
final class VaultManager: ObservableObject {
    ...
    /// Writes `text` as a new timestamped note in the configured subfolder.
    /// Safe to call from the background — pure local file I/O, no UI/app-switch.
    func saveNote(_ text: String) {
        ...
        let subfolderName = UserDefaults.standard.string(forKey: SettingsKeys.vaultSubfolder) ?? SettingsKeys.defaultVaultSubfolder
        let folderURL = vaultURL.appendingPathComponent(subfolderName, isDirectory: true)
        ...
        if let coordinatorError {
            ...
            errorMessage = "Couldn't write note: \(writeError.localizedDescription)"
        } else {
            errorMessage = nil
            lastSavedFilename = filename
            print("Saved note to \(fileURL.path)")
        }
    }
}
